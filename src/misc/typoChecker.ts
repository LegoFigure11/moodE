...
export class TypoChecker {
  message: Message;
  arg: string;
  pokemonList: string[] = this.generatePokemonList();
  ...
  getSimilarPokemon(): string[] {
    ...
    // TODO: Add logic to sort results by number of subs (least subs = closer match)
    for (const mon of this.pokemonList) {
      if (this.getLevenshteinValue(Utilities.toId(this.arg), mon) <= maxSubs) {
        results.push(mon);
      }
    }
    return results;
