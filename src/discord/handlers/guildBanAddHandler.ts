...
export class GuildBanAddHandler {
  eventsDirectory: string = path.join(Utilities.discordFolder, "events", "guildBanAdd");
  private events: KeyedDict<number, Dict<IGuildBanEvent>> = {};

  onReload(previous: Partial<GuildBanAddHandler>): void {
...
export const instantiate = (): void => {
  const oldHandler = global.GuildBanAddHandler as GuildBanAddHandler | undefined;

  global.GuildBanAddHandler = new GuildBanAddHandler();

  if (oldHandler) {
    global.GuildBanAddHandler.onReload(oldHandler);
  }
};
