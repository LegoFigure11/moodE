...
export class MessageReactionAddHandler {
  eventsDirectory: string = path.join(Utilities.discordFolder, "events", "messageReactionAdd");
  private events: KeyedDict<number, Dict<IMessageReactionEvent>> = {};

  onReload(previous: Partial<MessageReactionAddHandler>): void {
...
export const instantiate = (): void => {
  const oldHandler = global.MessageReactionAddHandler as MessageReactionAddHandler | undefined;

  global.MessageReactionAddHandler = new MessageReactionAddHandler();

  if (oldHandler) {
    global.MessageReactionAddHandler.onReload(oldHandler);
  }
};
