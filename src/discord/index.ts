import {green, grey} from "@colors/colors/safe";
...
ReadyChecker.on("loaded", () => {
  if (
    ...
    console.log(Utilities.discordText(green("Ready!")));
    __listen = true;
  }
});
...
client.on("ready", () => {
  ...
  for (const guild of guilds) {
    // Databases.getDatabase() creates a database if one doesn't already exist,
    // so there's no need to create them manually. Nice!
    Databases.getDatabase(guild[0]);
    const db = Databases.getDatabase(guild[0]);
    db.name = guild[1];
    Databases.exportDatabase(guild[0]);
  }
...
client.on(
  "messageCreate",
  (m) =>
    void (async (message: Discord.Message) => {
      if (__listen) {
        if (message.partial) {
          try {
            ...
            console.log(
              Utilities.discordText(
                `Unable to resolve a message partial! ${grey(
                  `${message.id}`
                )}`
              )
            );
        ...
        message = (await MessageCreateHandler.executeEvents(message).catch(
          console.error
        )) as unknown as Discord.Message;

        if (message) {
          const prefix =
            Databases.getDatabase(Utilities.toDatabaseId(message))?.prefix ||
            DiscordConfig.prefix!;
          message.content = message.content.trim();
          const u = client.user!.toString().replace("<@!", "<@");
          const startsWithBotTag = message.content
            .replace("<@!", "<@")
            .startsWith(u);
          if (message.content.startsWith(prefix) || startsWithBotTag) {
            ...
            const spacer = message.content
              .replace(firstPart, "")
              .startsWith(" ")
              ? " "
              : "";
            const commandName = message.content
              .replace(`${firstPart}${spacer}`, "")
              .split(" ")[0];
            const args = message.content
              .replace(`${firstPart}${spacer}${commandName}`, "")
              .split(",")
              .map((arg) => arg.trim());

            // Looks like we finally have everything in order. Lets run the command!
            CommandHandler.executeCommand(
              Utilities.toId(commandName),
              message,
              args
            );
          }
...
client.on(
  "guildBanAdd",
  (b) =>
    void (async (ban: Discord.GuildBan) => {
      if (__listen) {
        if (ban.partial) {
          try {
            ban = await ban.fetch();
            ...
            console.log(
              Utilities.discordText(
                `Unable to resolve a ban partial! ${grey(
                  `${ban.guild.id}`
                )}`
              )
            );
...
client.on(
  "guildMemberAdd",
  (m) =>
    void (async (member: Discord.GuildMember | Discord.PartialGuildMember) => {
      if (__listen) {
        if (member.partial) {
          try {
            ...
            console.log(
              Utilities.discordText(
                `Unable to resolve a member partial! ${grey(
                  `${member.id}`
                )}`
              )
            );
        ...
        GuildMemberAddHandler.executeEvents(
  ...
  (m) =>
    void (async (member: Discord.GuildMember | Discord.PartialGuildMember) => {
      if (__listen) {
        ...
        GuildMemberRemoveHandler.executeEvents(
          member as Discord.GuildMember
        ).catch(console.error);
      }
    })(m)
...
client.on(
  "messageDelete",
  (m) =>
    void (async (message: Discord.Message | Discord.PartialMessage) => {
      if (__listen) {
        if (message.partial) {
          try {
            message = await message.fetch();
        ...
        MessageDeleteHandler.executeEvents(message as Discord.Message).catch(
          console.error
        );
      }
    })(m)
      ...
      if (__listen) {
        if (user.partial) {
          try {
            ...
            console.log(
              Utilities.discordText(
                `Unable to resolve a user partial! ${grey(`${user.id}`)}`
              )
            );
          }
        }
        if (messageReaction.partial) {
          try {
            ...
            console.log(
              Utilities.discordText(`Unable to resolve a reaction partial!`)
            );
          }
        }
        MessageReactionAddHandler.executeEvents(
          messageReaction as Discord.MessageReaction,
