...
class CommandHandler {
	...
	async init(isReload) {
		console.log(`${Tools.twitchText()}${isReload ? "Rel" : "L"}oading commands...`);
		await Promise.all([
			this.loadDirectory(COMMANDS_DIRECTORY, Commands.TwitchCommand, "Bot", isReload),
	...
	loadDirectory(directory, Command, type, isReload) {
		console.log(`${Tools.twitchText()}${isReload ? "Rel" : "L"}oading ${type.cyan} commands...`);
					...
					for (let name of files) {
						if (name.endsWith(".js")) {
							try {
								...
								if (!(isReload)) console.log(`${Tools.twitchText()}${isReload ? "Rel" : "L"}oaded command ${type === "Private" ? (`${name.charAt(0)}*****`).green : name.green}`);
							} catch (e) {
								console.log(`${Tools.twitchText()}${"CommandHandler loadDirectory() error: ".brightRed}${e} while parsing ${name.yellow}${".js".yellow} in ${directory}`);
								console.log(e.stack);
							}
						}
					}
					console.log(`${Tools.twitchText()}${type.cyan} commands ${isReload ? "rel" : "l"}oaded!`);
					resolve();
				}
			});
		});
	...
	async executeCommand(message, channel, user, self, time) {
		let passDex = Dex;
		...
		for (let i = 0; i < commandsList.length; i++) {
			...
			if (command.trigger(cmd)) {
				...
				if (!isDeveloper(user)) {
					if (command.developerOnly) return bot.whisper(user.username, "You need to be a bot developer to use that command!");
					if (command.requiredRank && !hasBadge(user, command.requiredRank)) return bot.whisper(user.username, `You need to be a ${command.requiredRank} or higher to use ${twitchConfig.commandCharacter}${command.name} in ${channel.replace("#", "")}'s channel!'`);
					if (command.channels && !command.channels.includes(channel)) return bot.whisper(user.username, `The command ${twitchConfig.commandCharacter}${command.name} is not usable in ${channel.replace("#", "")}'s channel!`);
					if (command.noWhisper && user["message-type"] === "whisper") return bot.whisper(user.username, `The command ${twitchConfig.commandCharacter}${command.name} is not usable in whispers!`);
					if (command.whisperOnly && user["message-type"] !== "whisper") return bot.whisper(user.username, `The command ${twitchConfig.commandCharacter}${command.name} is only available in whispers!`);
				}
				...
				console.log(`${Tools.twitchText()}Executing command: ${command.name.cyan}`);
				try {
					if (command.commandType === "DexCommand") {
						await command.execute(args, channel, user, self, passDex);
					} else {
						await command.execute(args, channel, user, self);
					}
				} catch (e) {
					let stack = e.stack;
				...
				console.log(`${Tools.twitchText()}Executed command: ${command.name.green} in ${timeString}`);
			}
		}
	}
	...
	helpCommand(message, room, user, time) {
		const hrStart = process.hrtime();
		const commandsList = this.commands;
		console.log(`${Tools.twitchText()}Executing command: ${"help".cyan}`);
		const botCommands = [];
		const devCommands = [];
		const dexCommands = [];
		let sendMsg = [];
		...
		if (!(message.trim().includes(" "))) {
			user.say(`List of commands; use \`\`${twitchConfig.commandCharacter}help <command name>\`\` for more information:`);
			for (let i = 0; i < commandsList.length; i++) {
				const command = commandsList[i];
				if (!command.disabled && command.commandType !== "PrivateCommand") {
					const cmdText = `${twitchConfig.commandCharacter}${command.name}${command.desc ? ` - ${command.desc}` : ""}`;
			...
			const hrEnd = process.hrtime(hrStart);
			const timeString = hrEnd[0] > 3 ? `${hrEnd[0]}s ${hrEnd[1]}ms`.brightRed : `${hrEnd[0]}s ${hrEnd[1]}ms`.grey;
			console.log(`${Tools.twitchText()}Executed command: ${"help".green} in ${timeString}`);
...
function isDeveloper(user) {
	return twitchConfig.developers.includes(user.username);
}

function hasBadge(user, rank) {
	const userRanks = [];
	if (user.badges) {
		for (const badge of user.badges) userRanks.push(badge);
	}
	const groups = {
		"vip": 1,
		"founder": 2,
	...
	let hasRank = false;
	for (const group of userRanks) {
		if (groups[group] && groups[rank] && groups[group] >= groups[rank]) hasRank = true;
	}
	return hasRank;
}
